...
def run_cli(args):
    ...
    from core.scanner import PortScanner, ScanConfig, ScanType, PortState, parse_ports, parse_targets
    ...
    def on_port(target, pr):
        if pr.state == PortState.OPEN:
            ...
            print(f"  {target}:{pr.port}/{pr.protocol:<4} OPEN  {pr.service:<18} {pr.version[:25]}{banner}")
...
def main():
    ...
    parser.add_argument("-t", "--targets", default="",
                        help="Target(s): IP, hostname, CIDR, or range")
    ...
    parser.add_argument("--threads", type=int, default=300,
                        help="Number of threads (default: 300)")
